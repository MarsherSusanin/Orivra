...
describe("Slice 029B production promotion contracts", () => {
  it("exports one cycle-free production promotion feature through root identity", async () => {
    ...
    const source = await readFile(new URL("../src/production-promotion.ts", import.meta.url), "utf8");
    expect(source).not.toMatch(/from\s*["']\.\/index["']|from\s*["']@proofline\/contracts["']/);
    expect(source).not.toMatch(/\b(?:process|globalThis|fetch|readFile|writeFile|setTimeout|setInterval)\b/);
  });
