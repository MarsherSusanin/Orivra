...
test("accepts ADR 0045 and freezes 029D without claiming publication or deployment", async () => {
  ...
  assert.match(adr, /Status: Accepted boundary; post-publication deployer correction Author GREEN, independent reverification pending/);
  assert.match(slice, /Status: Post-publication deployer corrective Author GREEN; independent reverification pending; no final deployment acceptance/);
  assert.match(roadmap, /361bac3[\s\S]*obsolete\s+undeployable images[\s\S]*must not be published/i);
